...
but each column should have the same length.
...
#+begin_src lisp
  ...
  (let ((indexes (make-array '(10) :element-type '(unsigned-byte 8)
    ...
    (ddb:with-static-table ("primes" `(("i" . ,indexes)
                                       ("p" . ,primes)))
      (ddb:query "SELECT * FROM primes" nil)))
  ...
  (ddb:with-static-table ("bools" `(("v" . ((nil t :false :true :null)
    ...
    (ddb:query "SELECT v, v IS NULL AS is_null FROM bools" nil))
  ;; => (("v" . #(NIL T NIL T NIL)) ("is_null" . #(NIL NIL NIL NIL T)))
#+end_src
